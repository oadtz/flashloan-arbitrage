...
ACIDDatafeeds.UDFCompatibleDatafeed = function (datafeedURL, updateFrequency, protocolVersion, oauth, type, filters) {
    ...
    this._filters = filters == undefined ? this._defaultFilters : filters;
    this._acidToken = 'acid-token';
    this._defaultOAuth = {
        url: '/oauth2/token',
        client_id: '',
    ...
    this._oauth = oauth == undefined ? this._defaultOAuth : oauth;
    //console.log([this._filters, filters, oauth]);
    // End 


    this._barsPulseUpdater = new ACIDDatafeeds.DataPulseUpdater(this, updateFrequency || 10 * 1000);
    this._quotesPulseUpdater = new ACIDDatafeeds.QuotesPulseUpdater(this);
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.OAuthLogin = function () {
    //console.log(['login', this._oauth, this._acidToken]);
    var result = false;
    var data = this._oauth.grant_type == 'refresh_token'
               ? { client_id: this._oauth.client_id, grant_type: this._oauth.grant_type, refresh_token: this._oauth.refresh_token }
               : { client_id: this._oauth.client_id, grant_type: this._oauth.grant_type, username: this._oauth.username, password: this._oauth.password };
    $.ajax({
        type: 'POST',
        url: this._oauth.url,
        async: false,
        ...
        success: function (res) {
            //console.log(['ACIDDatafeeds.OAuthLogin.succes: ' , res, res.Status]);            
            result = res;
        }.bind(this),
        error: function (err) {
            console.log('ACIDDatafeeds.OAuthLogin.error: ' + err.responseText);
        }
    });
    localStorage.setItem(this._acidToken, JSON.stringify(result));
    return result;
};

ACIDDatafeeds.UDFCompatibleDatafeed.prototype.getOAuthToken = function () {
    return localStorage.getItem(this._acidToken);
};

ACIDDatafeeds.UDFCompatibleDatafeed.prototype.defaultConfiguration = function () {
    return {
        supports_search: true,
        supports_group_request: false,
        supported_resolutions: ['1', '5', '15', '30', '60', '1D', '1W', '1M'],//['1', '5', '15', '30', '60', '1D', '1W', '1M', '3M', '6M', '1Y', '3Y', '5Y'],
        supports_marks: false,
        supports_timescale_marks: true
    };
};

ACIDDatafeeds.UDFCompatibleDatafeed.prototype.getServerTime = function (callback) {
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.on = function (event, callback) {
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype._fireEvent = function (event, argument) {
    if (this._callbacks.hasOwnProperty(event)) {
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.onInitialized = function () {
    this._initializationFinished = true;
    this._fireEvent('initialized');
};

ACIDDatafeeds.UDFCompatibleDatafeed.prototype._logMessage = function (message) {
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype._send = function (url, params) {
    var request = url;
    if (params) {
    ...
    var bearerToken = JSON.parse(this.getOAuthToken());
    if (bearerToken == '' || bearerToken == undefined) {
        bearerToken = this.OAuthLogin();
    }
    ...
    return $.ajax({
        ...
        success: function (res) {
            //console.log('_send.success', res);
        },
        error: function (err) {
            console.log('_send.error', err);
            if (err.status == 401) {
                // If HttpStatusCode = 401 (Unauthorized), resubmit request
                this.OAuthLogin();
                console.log(['Unauthorized: resubmit ajax call', request, params]);
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype._initialize = function () {
    var that = this;
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.onReady = function (callback) {
    var that = this;
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype._setupWithConfiguration = function (configurationData) {
    ...
    if (!configurationData.supports_search) {
        this._symbolSearch = new ACIDDatafeeds.SymbolSearchComponent(this);
    }

    if (configurationData.supports_group_request) {
        //	this component will call onInitialized() by itself
        this._symbolsStorage = new ACIDDatafeeds.SymbolsStorage(this);
    } else {
        this.onInitialized();
    }
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.getMarks = function (symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.getTimescaleMarks = function (symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.searchSymbols = function (searchString, exchange, type, onResultReadyCallback) {
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype._symbolResolveURL = '/symbols';

//	BEWARE: this function does not consider symbol's exchange
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    var that = this;
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype._historyURL = '/history';

ACIDDatafeeds.UDFCompatibleDatafeed.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
    ...
    this._send(this._datafeedURL + this._historyURL, {
        ...
        to: rangeEndDate,
        // custom property
        symbolType: this._symbolType,//symbolInfo.type,	    
        carriers: this._filters.carriers,
        stations: this._filters.stations,
        substations: this._filters.substations,
        uldgroups: this._filters.uldgroups,
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.subscribeBars = function (symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback) {
    this._barsPulseUpdater.subscribeDataListener(symbolInfo, resolution, onRealtimeCallback, listenerGUID, onResetCacheNeededCallback);
};

ACIDDatafeeds.UDFCompatibleDatafeed.prototype.unsubscribeBars = function (listenerGUID) {
    this._barsPulseUpdater.unsubscribeDataListener(listenerGUID);
};

ACIDDatafeeds.UDFCompatibleDatafeed.prototype.calculateHistoryDepth = function (period, resolutionBack, intervalBack) {
};

ACIDDatafeeds.UDFCompatibleDatafeed.prototype.getQuotes = function (symbols, onDataCallback, onErrorCallback) {
...
ACIDDatafeeds.UDFCompatibleDatafeed.prototype.subscribeQuotes = function (symbols, fastSymbols, onRealtimeCallback, listenerGUID) {
    this._quotesPulseUpdater.subscribeDataListener(symbols, fastSymbols, onRealtimeCallback, listenerGUID);
};

ACIDDatafeeds.UDFCompatibleDatafeed.prototype.unsubscribeQuotes = function (listenerGUID) {
...
*/
ACIDDatafeeds.SymbolsStorage = function (datafeed) {
...
ACIDDatafeeds.SymbolsStorage.prototype._requestFullSymbolsList = function () {
    var that = this;
...
ACIDDatafeeds.SymbolsStorage.prototype._onExchangeDataReceived = function (exchangeName, data) {
...
ACIDDatafeeds.SymbolsStorage.prototype._onAnyExchangeResponseReceived = function (exchangeName) {
...
//	BEWARE: this function does not consider symbol's exchange
ACIDDatafeeds.SymbolsStorage.prototype.resolveSymbol = function (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    var that = this;
...
ACIDDatafeeds.SymbolSearchComponent = function (datafeed) {
    this._datafeed = datafeed;
};

//	searchArgument = { searchString, onResultReadyCallback}
ACIDDatafeeds.SymbolSearchComponent.prototype.searchSymbols = function (searchArgument, maxSearchResults) {
...
ACIDDatafeeds.DataPulseUpdater = function (datafeed, updateFrequency) {
    this._datafeed = datafeed;
    this._subscribers = {};
...
ACIDDatafeeds.DataPulseUpdater.prototype.unsubscribeDataListener = function (listenerGUID) {
    this._datafeed._logMessage('Unsubscribing ' + listenerGUID);
    delete this._subscribers[listenerGUID];
};

ACIDDatafeeds.DataPulseUpdater.prototype.subscribeDataListener = function (symbolInfo, resolution, newDataCallback, listenerGUID) {
...
ACIDDatafeeds.DataPulseUpdater.prototype.periodLengthSeconds = function (resolution, requiredPeriodsCount) {
    var daysCount = 0;
...
ACIDDatafeeds.QuotesPulseUpdater = function (datafeed) {
    this._datafeed = datafeed;
    this._subscribers = {};
...
ACIDDatafeeds.QuotesPulseUpdater.prototype.subscribeDataListener = function (symbols, fastSymbols, newDataCallback, listenerGUID) {
    if (!this._subscribers.hasOwnProperty(listenerGUID)) {
        this._subscribers[listenerGUID] = {
            symbols: symbols,
...
ACIDDatafeeds.QuotesPulseUpdater.prototype.unsubscribeDataListener = function (listenerGUID) {
    delete this._subscribers[listenerGUID];
};

ACIDDatafeeds.QuotesPulseUpdater.prototype._updateQuotes = function (symbolsGetter) {
