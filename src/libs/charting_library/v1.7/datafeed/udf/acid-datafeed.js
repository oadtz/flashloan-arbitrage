...
Datafeeds.UDFCompatibleDatafeed = function (datafeedURL, updateFrequency, oauth, type, filters, data, webApiURL) {
    this._enableLogging = Acid.debugModeOn;
...
Datafeeds.UDFCompatibleDatafeed.prototype.OAuthLogin = function () {
    var result = false;
    ...
    $.ajax({
        ...
        error: function (err) {
            console.log('Datafeeds.OAuthLogin.error: ' + err.responseText);
        }
    });
    try {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getOAuthToken = function () {
...
Datafeeds.UDFCompatibleDatafeed.prototype._send = function (url, params) {
    ...
    this._logMessage('acid-datafeed.js: New request: ' + request);
...
Datafeeds.UDFCompatibleDatafeed.prototype._post = function (url, params) {
    this._logMessage(['acid-datafeed.js: New request (post):', url, JSON.stringify(params)]);
...
Datafeeds.UDFCompatibleDatafeed.prototype._setupWithConfiguration = function (configurationData) {
    ...
    this._logMessage('acid-datafeed.js: Initialized with ' + JSON.stringify(configurationData));
};
...
Datafeeds.UDFCompatibleDatafeed.prototype.getTimescaleMarks = function (symbolInfo, rangeStart, rangeEnd, onDataCallback, resolution) {
    if (this._configuration.supports_timescale_marks) {
        var discussionData = this._data.discussionData;
        var timescaleMarks = this._data.timescaleMarks;
        ...
        this._send(this._datafeedURL + '/timescale_marks', {
			...
			.done(function (response) {
                ...
                if (timescaleMarks) {
                    for (var i = 0; i < timescaleMarks.length; i++) {
                        response.push(timescaleMarks[i]);
                    }
                }
                ...
                if (discussionData) {
                    ...
                    $.ajax({
                        url: window.discussion_api_url + 'api/post/links',
                        async: false,
                        data: {
                            hash: JSON.stringify({
                                _carrierId: discussionData._carrierId !== undefined && discussionData._carrierId !== null ? parseInt(discussionData._carrierId) : null,
                                _stationId: discussionData._stationId !== undefined && discussionData._stationId !== null ? parseInt(discussionData._stationId) : null,
                                _uldGroupId: discussionData._uldGroupId !== undefined && discussionData._uldGroupId !== null ? parseInt(discussionData._uldGroupId) : null
                            })
                        },
                        type: 'GET',
                        dataType: 'json',
                        success: function (posts) {
                            if (Array.isArray(posts)) {
                                for (var i = 0; i < posts.length; i++) {
                                    // Dynamically add discussion link to timeline
                                    response.push({
                                        id: 'discussion:' + posts[i]._id,
                                        label: 'i',
                                        color: posts[i].status == 0 ? '#00f' : posts[i].status == 1 ? '#f00' : '#0f0',
                                        time: moment(posts[i].hash._eventDateTime || posts[i].createdDateTime).unix(),
                                        tooltip: [posts[i].statusDetail.text, posts[i].createdDateTime, '[' + posts[i].createdUser.login + '] created post:', posts[i].subject]
                                    });
                                }
                            }
                        },
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function (symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    ...
    // Customized: remove exchange from symbol as we are currently not support
    ...
    that._logMessage('acid-datafeed.js: Resolve requested');
    ...
    function onResultReady(data) {
        ...
        that._logMessage('acid-datafeed.js: Symbol resolved: ' + (Date.now() - resolveRequestStartTime));
        ...
        this._send(this._datafeedURL + this._symbolResolveURL, {
            ...
            .done(function (response) {
                ...
                if (data.s && data.s !== 'ok') {
                    ...
                    if (tokens.length > 1) {
                        //data.ticker = data.ticker + '#' + tokens[1];    // add '#...' back to the symbol
                        data.ticker = tokens[0] + '#' + tokens[1];    // add '#...' back to the symbol 
                    }
            ...
            .fail(function (reason) {
                that._logMessage('acid-datafeed.js: Error resolving symbol: ' + JSON.stringify([reason]));
                onResolveErrorCallback('unknown_symbol');
            });
    } else {
        if (this._initializationFinished) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
    ...
    var tokens = symbolInfo.ticker.split('#');
    var indicatorName = null;
    if (tokens.length > 1) {
        indicatorName = tokens[1];
        var s = tokens[1].substring(0, 8);

        ticker = tokens[0]; //symbolInfo.ticker = tokens[0];              // set the symbol back to its original
        symbolInfo.indicatorName = indicatorName;   // keep the indicator name
    }

    /// extend into the future by half of the date range if the end date is in a future
    /// ex: if date range is 360 days, then extend the end date by 180 days
    if (moment().unix() <= rangeEndDate) {
        var periodLengthSeconds = rangeEndDate - rangeStartDate;
        rangeEndDate += parseInt(periodLengthSeconds/2);
    }
    ...
    if (true) {
        ...
        this._post(this._datafeedURL + this._historyURL, {
            symbol: ticker,
            customIndicatorName: symbolInfo.indicatorName,
            resolution: resolution,
            from: rangeStartDate,
            to: rangeEndDate,
            // custom property
            ...
            networkcarriers: this._filters.networkcarriers,
            ...
            isnetworkstock: this._filters.isnetworkstock,
            scopes: this._filters.scopes
        })
            .done(function (response) {
                var data = JSON.parse(response);
                ...
                for (var i = 0; i < barsCount; ++i) {
                    ...
                    if (moment().unix() <= data.t[i]) {
                        barValue.open = barValue.high = barValue.low = barValue.close = undefined;
                    } else if (ohlPresent) {
                    ...
                    if (volumePresent) {
                        ...
                        if (data.v[i].Kpi && !data.v[i].Kpi.Css && i > 0) {
                            barValue.volume.Kpi.Css = data.v[i - 1].Kpi.Css;
                        }
                        if (data.v[i].Kpi && !data.v[i].Kpi.Ops && i > 0) {
                            barValue.volume.Kpi.Ops = data.v[i - 1].Kpi.Ops;
                        }
                        if (data.v[i].Kpi && !data.v[i].Kpi.Min && i > 0) {
                            barValue.volume.Kpi.Min = data.v[i - 1].Kpi.Min;
                        }
                        if (data.v[i].Kpi && !data.v[i].Kpi.Max && i > 0) {
                            barValue.volume.Kpi.Max = data.v[i - 1].Kpi.Max;
                        }
                    }

                    // Check if there is data point to be inserted passed
                    //if (that._data.dataPoint && data.t[i] > that._data.dataPoint.t && !inserted && i > 0) {
                    if (!dataPointInserted && that._data.dataPoint && (i == barsCount - 1 || (that._data.dataPoint.t >= data.t[i] && that._data.dataPoint.t < data.t[i + 1]))) {
                        //console.log('inserting new data point');
                        barValue.time = that._data.dataPoint.t * 1000;
                        barValue.close = that._data.dataPoint.c;
                        if (ohlPresent) {
                            barValue.high = that._data.dataPoint.c > barValue.high ? that._data.dataPoint.c : barValue.high;
                            barValue.low = that._data.dataPoint.c < barValue.low ? that._data.dataPoint.c : barValue.low;
                        }

                        dataPointInserted = true;
                    }
...
Datafeeds.SymbolsStorage.prototype._onAnyExchangeResponseReceived = function (exchangeName) {
    ...
    if (allDataReady) {
        ...
        this._datafeed._logMessage('acid-datafeed.js: All exchanges data ready');
...
Datafeeds.DataPulseUpdater.prototype.unsubscribeDataListener = function (listenerGUID) {
    this._datafeed._logMessage('acid-datafeed.js: Unsubscribing ' + listenerGUID);
    delete this._subscribers[listenerGUID];
};

Datafeeds.DataPulseUpdater.prototype.subscribeDataListener = function (symbolInfo, resolution, newDataCallback, listenerGUID) {
    this._datafeed._logMessage('acid-datafeed.js: Subscribing ' + listenerGUID);
