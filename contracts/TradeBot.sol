...
contract TradeBot is Ownable {
        ...
        )
    {
        ...
        TradeData memory ethToTokenResult = getTradeAnalysis(
            routers,
            WETH,
            token,
            gasCostLimitInWei
        );
        uint256 ethToTokenProfitRatio = ethToTokenResult.amountOut > 0
            ? (ethToTokenResult.profit * 100) / ethToTokenResult.amountOut
            : 0;
        ...
        TradeData memory tokenToETHResult = getTradeAnalysis(
            routers,
            token,
            WETH,
            gasCostLimitInWei
        );
        uint256 tokenToETHProfitRatio = tokenToETHResult.amountOut > 0
            ? (tokenToETHResult.profit * 100) / tokenToETHResult.amountOut
            : 0;
        ...
        if (ethToTokenProfitRatio > tokenToETHProfitRatio) {
            ...
            return (
                "eth_to_token",
                ethToTokenResult.bestRouter,
                ethToTokenResult.amountIn,
                ethToTokenResult.amountOut
            );
        } else if (tokenToETHProfitRatio > ethToTokenProfitRatio) {
            // amoutIn is in Token, amountOut is in ETH, profit is in ETH
            return (
                "token_to_eth",
                tokenToETHResult.bestRouter,
                tokenToETHResult.amountOut,
                tokenToETHResult.amountIn
            );
        } else {
    ...
    function getTradeAnalysis(
        ...
        uint256 currentTokenInBalance = tokenIn == WETH
            ? address(this).balance
            : IERC20(tokenIn).balanceOf(address(this));
        // console.log("Current tokenIn in balance:", currentTokenInBalance);
        uint256 lastTradeOfTokenIn = tokenIn == WETH
            ? _trades[tokenOut].amountETH
            : _trades[tokenIn].amountToken;
        ...
        tradeData.amountIn = lastTradeOfTokenIn > 0
            ? min(currentTokenInBalance, lastTradeOfTokenIn)
        ...
        if (tradeData.amountIn > 0) {
            for (uint8 i = 0; i < routers.length; i++) {
                ...
                if (amountOut > tradeData.amountOut) {
                    ...
                    uint256 gasCostLimit = tokenOut == WETH
                        ? gasCostLimitInWei
                        : (gasCostLimitInWei * amountOut) / tradeData.amountIn;
                    // console.log("Gas cost limit for router:", i, gasCostLimit);
                    tradeData.profit = amountOut >
                        gasCostLimit + expectedAmountOut
                        ? amountOut - gasCostLimit - expectedAmountOut
                        : 0;
    ...
    function executeTradeETHForTokens(
        ...
        IRouter(router).swapExactETHForTokens{value: amountIn}(
            amountOutMin,
            path,
            address(this),
            block.timestamp
    ...
    function resetTokenTrade(address token) external onlyOwner {
